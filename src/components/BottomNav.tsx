...
export function BottomNav() {
  ...
  return (
    <nav aria-label="Navigazione principale" className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-background/95 backdrop-blur-xl">
          ...
          if (isAdd) {
            return (
              <motion.div key={i} whileTap={{ scale: 0.88 }} style={{ marginTop: -8 }}>
                <Link href="/add" aria-label="Aggiungi raccomandazione" className="flex items-center justify-center">
                  <span aria-hidden="true" className="flex h-[52px] w-[52px] items-center justify-center rounded-full bg-primary shadow-[0_4px_24px_rgba(13,148,136,0.5)] transition-transform">
            ...
            <motion.div key={i} whileTap={{ scale: 0.82 }} className="flex flex-col items-center">
              <Link
                ...
                className={`flex flex-col items-center gap-[3px] px-2 transition-colors ${
                  active ? "text-primary" : "text-muted-foreground hover:text-subdued"
                }`}
              >
              ...
              <AnimatePresence>
                {active && (
                  <motion.div
                    ...
                    className="mt-[3px] h-[3px] w-[3px] rounded-full bg-primary"
                  />
                )}
              </AnimatePresence>
            </motion.div>
