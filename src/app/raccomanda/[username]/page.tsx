...
export async function generateMetadata({
  ...
  const { data: profile } = await supabase
    ...
    .single();
  const name = (profile as { full_name?: string | null } | null)?.full_name ?? username;
  return {
    title: `Raccomanda ${name} su Filo`,
    description: `${name} ti invita a raccomandarli su Filo — il social network del passaparola professionale.`,
  };
}

export default async function RaccomandaPage({
  params,
}: {
  params: Promise<{ username: string }>;
}) {
  ...
  const fullName = (profile as { full_name?: string | null }).full_name ?? "Utente";
  const city = (profile as { city?: string | null }).city ?? null;
  const category = (profile as { professional_category?: string | null }).professional_category ?? null;
  const color = avatarColor(profile.id);

  // Fetch how many recs this professional has already received
