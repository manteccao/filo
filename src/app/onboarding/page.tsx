  ...
  return (
    <div className="flex items-center gap-1.5">
      {([1, 2, 3] as Step[]).map((s) => (
        <div
          key={s}
          className={`h-1 flex-1 rounded-full transition-all duration-300 ${s <= step ? "bg-primary" : "bg-muted"}`}
        />
      ))}
    </div>
  );
...
export default function OnboardingPage() {
  ...
  return (
    <div className="flex min-h-svh flex-col items-center justify-center bg-background px-6 py-12 text-white">
      <div className="w-full max-w-sm">
        <ProgressBar step={step} />
        <p className="mt-2 text-right text-xs text-muted-foreground">{step} di 3</p>
          ...
          <div className="mt-8">
            ...
            <div className="mt-8 flex flex-col gap-3">
              <button
                ...
                className="flex items-center gap-4 rounded-2xl border border-border bg-card p-4 text-left transition hover:border-primary/50 active:scale-[0.98]"
              >
        ...
        {step === 2 && accountType === "user" && (
          <div className="mt-8">
            ...
            <form onSubmit={handleAddRec} className="mt-6 space-y-3">
              ...
              <div>
                <label className="mb-1.5 block text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">
                  La tua città <span className="text-primary">*</span>
                </label>
                <CityAutocomplete
                  value={myCity}
                  onChange={setMyCity}
              ...
              <div className="pt-2">
                <p className="mb-3 text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">
                  ...
                  <span className="normal-case font-normal text-muted-foreground">(opzionale)</span>
                </p>
              ...
              <button
                ...
                className="h-12 w-full rounded-2xl bg-primary text-sm font-semibold text-white shadow-[0_0_24px_rgba(13,148,136,0.3)] transition hover:bg-primary-hover disabled:opacity-50 active:scale-[0.98]"
              >
            ...
            <button
              ...
              disabled={saving || !myCity.trim()}
              className="mt-3 w-full text-center text-sm text-muted-foreground transition hover:text-muted-foreground disabled:opacity-40"
            >
        ...
        {step === 3 && accountType === "user" && (
          <div className="mt-10 flex flex-col items-center text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-whatsapp/15">
            ...
            <a
              ...
              className="mt-8 flex h-12 w-full items-center justify-center gap-2.5 rounded-2xl bg-whatsapp text-sm font-semibold text-white shadow-[0_0_24px_rgba(37,211,102,0.25)] transition hover:bg-whatsapp-hover active:scale-[0.98]"
            >
            ...
            <Link href="/feed" className="mt-4 text-sm text-muted-foreground transition hover:text-muted-foreground">
              Vai al feed →
            </Link>
          </div>
        )}
        ...
        {step === 3 && accountType === "professional" && (
          <div className="mt-8 flex flex-col items-center text-center">
            ...
            {/* Link box */}
            <div className="mt-6 w-full rounded-2xl border border-border bg-card px-4 py-3">
              <p className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">
                Il tuo link personale
              </p>
              <p className="mt-1.5 break-all text-[13px] font-medium text-primary">
                {proInviteUrl}
              </p>
            </div>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-4 flex h-12 w-full items-center justify-center gap-2.5 rounded-2xl bg-whatsapp text-sm font-semibold text-white shadow-[0_0_24px_rgba(37,211,102,0.25)] transition hover:bg-whatsapp-hover active:scale-[0.98]"
            >
              <svg viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
              </svg>
